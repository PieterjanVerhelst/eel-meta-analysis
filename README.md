...
14. `analyse_migration_period.R`: Analyse the moment (day of year) when eels started to migrate and the moment when they reached the sea in relation to size and geographical location
15. `analyse_escapement_success.R`: Analyse the succesful escapement to the sea according to migration barrier types, including fishing, and number
16. `calculate_migration_speed_overall.R`: Calculate the migration speed as the speed between the first and last detection and relate it to geographical position, habitat, size and sex
17. `calculate_migration_speed_habitats.R`: Calculate the migration speed according to habitat type and analyse difference between tidal vs non-tidal habitats
18. `analyse_migration_speed_tidal.R`: Analyse the migration speed in **tidal areas** according to size, sex and geographical location
19. `analyse_migration_speed_nontidal.R`: Analyse the migration speed in **non-tidal areas** according to size, sex, geographical location, migration barrier number and type
